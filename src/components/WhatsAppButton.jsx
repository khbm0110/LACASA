...
export default function WhatsAppButton() {
  ...
  return (
    <motion.div className="fixed bottom-5 left-5 z-50" initial={{opacity:0,scale:0}} animate={{opacity:1,scale:1}} transition={{delay:1.5,type:"spring",stiffness:260,damping:20}}>
      <AnimatePresence>
        {showTip && (
          <motion.div initial={{opacity:0,x:10}} animate={{opacity:1,x:0}} exit={{opacity:0,x:10}} className="absolute bottom-full left-0 mb-3 bg-surface border border-border p-3.5 whitespace-nowrap">
            <p className="text-ink text-sm font-medium">Besoin d aide ?</p>
            <p className="text-inkMuted text-xs">WhatsApp</p>
          </motion.div>
        )}
      </AnimatePresence>
      <motion.a href={`https://wa.me/${number.replace(/\D/g,"")}?text=${msg}`} target="_blank" rel="noreferrer" aria-label="WhatsApp" className="w-12 h-12 bg-[#25D366] flex items-center justify-center block cursor-pointer" whileHover={{scale:1.1}} whileTap={{scale:0.9}}>
